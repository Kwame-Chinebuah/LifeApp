      ...
      {adding ? (
        <View style={styles.form}>
          <TextInput style={styles.input} value={siteName} onChangeText={setSiteName}
            placeholder="Site / App name (e.g. Asda Rewards)" placeholderTextColor={COLORS.textTertiary} />
          <TextInput style={styles.input} value={username} onChangeText={setUsername}
            placeholder="Username / Email (optional)" placeholderTextColor={COLORS.textTertiary} />
          <View style={styles.pwInput}>
            <TextInput style={[styles.input, { flex: 1 }]} value={password} onChangeText={setPassword}
              placeholder="Password" placeholderTextColor={COLORS.textTertiary} secureTextEntry />
            <TouchableOpacity style={styles.genBtn} onPress={gen}>
              <Text style={styles.genBtnText}>Generate</Text>
            </TouchableOpacity>
          </View>
          {/* Generator options */}
          <View style={styles.genOpts}>
            {[['upper','ABC'],['numbers','123'],['symbols','!@#']].map(([k,l]) => (
              <TouchableOpacity key={k} style={[styles.optBtn, opts[k] && styles.optBtnActive]} onPress={() => toggleOpt(k)}>
...
const styles = StyleSheet.create({
  ...
  card: {
    ...
    borderRadius: RADIUS.lg, borderWidth: 0.5, borderColor: COLORS.border, gap: 6,
  },
  ...
  cardPw: { fontSize: 15, color: COLORS.textPrimary, fontFamily: 'monospace', flex: 1 },
  ...
  pwInput: { flexDirection: 'row', gap: 10, alignItems: 'center' },
  genBtn: { padding: 12, backgroundColor: COLORS.bgTertiary, borderRadius: RADIUS.md },
  genBtnText: { fontSize: 13, color: COLORS.textSecondary },
  genOpts: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  optBtn: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: RADIUS.sm, borderWidth: 0.5, borderColor: COLORS.border },
  optBtnActive: { backgroundColor: COLORS.accentLight, borderColor: COLORS.accent },
  optBtnText: { fontSize: 12, color: COLORS.textSecondary },
  optBtnTextActive: { color: COLORS.accentText },
  stepBtn: { width: 28, height: 28, borderRadius: 14, borderWidth: 0.5, borderColor: COLORS.border, alignItems: 'center', justifyContent: 'center' },
  stepBtnText: { fontSize: 16, color: COLORS.textPrimary },
  lenText: { fontSize: 14, fontWeight: '500', color: COLORS.textPrimary, minWidth: 24, textAlign: 'center' },
